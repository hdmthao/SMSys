...
vector<Student> Staff::GetStudentListofCourse(string &course_name)
{
	vector<Student> students_list;
	Helper::StringToUpper(course_name);
	string path = Path::COURSE + course_name + "/student_info.txt";
	ifstream fin(path, ios::app);
	if (fin.is_open()) {
		while (!fin.eof()) {
			string data_line;
...
vector<Attendance> Staff::GetAttendanceList(string &course_name)
{
	vector<Attendance> attendance_list;
	Helper::StringToUpper(course_name);
	string path = Path::COURSE + course_name + "/attendance.txt";
	ifstream fin(path, ios::app);
	if (fin.is_open()) {
		while (!fin.eof()) {
			string data_line;
