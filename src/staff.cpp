...
bool Staff::EditAttendance(string course_id, Attendance a)
{
	bool done = false;
	string path = Path::COURSE + course_id + "/attendance.txt";
	...
	if (in.is_open()) {
		while (!in.eof()) {
			...
			attendance = Helper::stringToAttendance(data_line);
			if (attendance.ID == 0) break;
			if (attendance.ID == a.ID)
			{
				attendance.week = a.week;
				done = true;
			}
	...
	for (int i = 0; i < AttendanceList.size(); i++) {
		out << AttendanceList[i].ID << " " << AttendanceList[i].first_name << " " << AttendanceList[i].last_name << " ";
		for (int j = 0; j < 10; j++)
		{
			if (AttendanceList[i].week[j] == 2)
				out << "1 1 ";
			if (AttendanceList[i].week[j] == 1)
				out << "1 0 ";
			if (AttendanceList[i].week[j] == 0)
				out << "0 0 ";
		}
		out << std::endl;
	}
...
bool Staff::EditGradeOfStudent(string course_id, Score a)
{
	bool done = false;
	vector<Score> ScoreBoard;
	Helper::StringToUpper(course_id);
